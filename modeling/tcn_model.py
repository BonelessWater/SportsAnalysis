...
def main():
    ...
    # Instantiate TCN model (if available)
    try:
        from pytorch_forecasting.models.tcn import TemporalConvolutionalNetwork
        tcn_model = TemporalConvolutionalNetwork.from_dataset(
            training,
            learning_rate=0.03,
            loss=QuantileLoss(),
            log_interval=10,
        ...
        print("TCN model is not available in your PyTorch Forecasting version.")
        return
    
    # Set up trainer and train
    trainer = pl.Trainer(max_epochs=1, accelerator="gpu", devices=1, num_sanity_val_steps=0)
    trainer.fit(tcn_model, train_dataloader, val_dataloaders=val_dataloader)
    
    # Make predictions
    future_data = TimeSeriesDataSet.from_dataset(training, data, predict=True)
    predictions = tcn_model.predict(future_data)
    print("TCN Model Predictions:")
    print(predictions)
