...
def run_forecasting_model(model_name: str, data_path: str, max_epochs: int = 5, batch_size: int = 2):
    ...
    # Build dataset (for simplicity the same dataset is used for all models)
    # You may adjust parameters based on model requirements.
    ...
    if model_name == "Baseline":
        ...
        # For the custom LSTM model, we need to determine the input size.
    ...
    # Prediction: for our custom LSTM we need to iterate over the batches manually.
    predictions = []
    model.eval()
    with torch.no_grad():
...
def main():
    ...
    parser.add_argument("--model", type=str, default="LSTM", help=("Model abbreviation to use. Options are: "
                                                                  "TFT, DAR, NBE, NHI, LSTM, DMLP, RN, BL"))
