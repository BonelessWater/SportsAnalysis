...
from pytorch_forecasting import NBeats, TimeSeriesDataSet, QuantileLoss
...
def main():
    ...
    trainer.fit(nbeats_model, train_dataloader, val_dataloaders=val_dataloader)
    
    # Make predictions
    future_data = TimeSeriesDataSet.from_dataset(training, data, predict=True)
    predictions = nbeats_model.predict(future_data)
    print("N-BEATS Model Predictions:")
    print(predictions)
